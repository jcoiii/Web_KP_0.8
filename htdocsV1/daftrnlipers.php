...
tr:nth-child(even) {
 ...
 <a class="navbar-brand" href="homeadmin.php">Beranda</a>		
   ...
   <br>
        <br>
 <h1 class="h3 mb-3 font-weight-normal">Daftar Nilai Perusahaan </h1>     
<br>
...
<?php		
 ...
 $con=mysqli_connect('localhost','root','') or die(mysqli_error());
      mysqli_select_db($con, 'nilai') or die("cannot select DB"); 
        $sql = "SELECT * FROM n_perusahaan";
        $result = $con->query($sql);
        
    if ($result->num_rows > 0) {
    echo "<table><tr><th>Kelompok</th><th>Nilai</th><th>Komentar</th></tr>";
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<tr><td>" . $row["kelompok"]. "</td><td>" . $row["nilai"]. "</td><td>" . $row["komentar"]. "</td></tr>";
    }
    echo "</table>";
} else {
